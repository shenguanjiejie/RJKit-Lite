...
- (NSInteger)dayIntervalSinceNow{
    return [self dayIntervalWithDate:[NSDate date]];
}

- (NSInteger)dayIntervalWithDate:(NSDate *)date{
    return (NSInteger)[self timeIntervalSinceDate:date] / (60 * 60 * 24);
}

- (NSInteger)hourIntervalSinceNow{
    return [self hourIntervalWithDate:[NSDate date]];
}

- (NSInteger)hourIntervalWithDate:(NSDate *)date{
    return ((NSInteger)[self timeIntervalSinceDate:date] % (60 * 60 * 24)) / (60 * 60);
}

- (NSInteger)minuteIntervalSinceNow{
    return [self minuteIntervalWithDate:[NSDate date]];
}

- (NSInteger)minuteIntervalWithDate:(NSDate *)date{
    return (((NSInteger)[self timeIntervalSinceDate:date] % (60 * 60 * 24)) % (60 * 60)) / 60;
}

- (NSInteger)secondIntervalSinceNow{
    return [self secondIntervalWithDate:[NSDate date]];
}
- (NSInteger)secondIntervalWithDate:(NSDate *)date{
    return (((NSInteger)[self timeIntervalSinceDate:date] % (60 * 60 * 24)) % (60 * 60)) % 60;
}

- (NSString *)descriptionIntervalSinceNow{
    return [NSString stringWithFormat:@"%02zd:%02zd:%02zd",[self hourIntervalSinceNow],[self minuteIntervalSinceNow],[self secondIntervalSinceNow]];
}

- (NSString *)descriptionIntervalWithDate:(NSDate *)date{
    return [NSString stringWithFormat:@"%02zd:%02zd:%02zd",[self hourIntervalWithDate:date],[self minuteIntervalWithDate:date],[self secondIntervalWithDate:date]];
}
...
//        return [NSString stringWithFormat:@"%.0f分钟前", retTime];
        return retTime < 1.0 ? kLocalString(just_now) : [NSString stringWithFormat:@"%.0f%@", retTime,kLocalString(m_before)];

    } else if (time < 3600 * 24) { // 小于一天，也就是今天
        retTime = time / 3600;
        retTime = retTime <= 0.0 ? 1.0 : retTime;
        return [NSString stringWithFormat:@"%.0f%@", retTime,kLocalString(h_before)];
    } else if (time < 3600 * 24 * 2) {
        return kLocalString(yesterday);
    }
    // 第一个条件是同年，且相隔时间在一个月内
    // 第二个条件是隔年，对于隔年，只能是去年12月与今年1月这种情况
        ...
        return [NSString stringWithFormat:@"%d%@", (abs)(retDay),kLocalString(d_before)];
    } else  {
        if (abs(year) <= 1) {
            if (year == 0) { // 同年
                return [NSString stringWithFormat:@"%d%@", abs(month),kLocalString(M_before)];
            }
            ...
            if (month == 12 && preMonth == 12) {// 隔年，但同月，就作为满一年来计算
                return [@"1" stringByAppendingString:kLocalString(y_before)];
            }
            return [NSString stringWithFormat:@"%d%@", (abs)(12 - preMonth + month),kLocalString(M_before)];
        }
        
        return [NSString stringWithFormat:@"%d%@", abs(year),kLocalString(y_before)];
    }
    
    return [@"1" stringByAppendingString:kLocalString(h_before)];
}
