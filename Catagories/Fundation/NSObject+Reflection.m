...
+ (NSDictionary *)dictionaryWithProperty:(objc_property_t)property
{
    ...
    if ([attributeDictionary objectForKey:@"T"])
    {
        ...
        if (type_str == nil)
        {
            ...
            {
                id str = [key substringWithRange:NSMakeRange(2, key.length - 3)];
                type_str = [str stringByAppendingString:@"*"];
                /**shenruijie 2018-02-08 16:35:35 为初始化一个对象铺路*/
                [result setObject:str forKey:result[@"name"]];
            }
...
//    链接：https://www.jianshu.com/p/d361f169423b
//    NSMutableArray *propertieArray = [NSMutableArray array];
//    
//    unsigned int propertyCount;
//    objc_property_t *properties = class_copyPropertyList([self class], &propertyCount);
...
//        NSString *result = [NSString stringWithCString:type encoding:NSUTF8StringEncoding];
//        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
//            
//            class_getProperty([self class], <#const char * _Nonnull name#>)
...
//    free(ivars);
    ...
    NSArray<NSDictionary *> *propertys = [self propertiesInfo];
    for (NSDictionary *info in propertys) {
//        NSString *propertyType = info[@"type"];
        NSString *propertyName = info[@"name"];
        NSString *attribute = [info[@"attribute"] firstObject];
        NSString *classStr = info[propertyName];
        if (classStr && NSClassFromString(classStr) && ![attribute isEqualToString:@"readonly"]) {
            [self setValue:[[NSClassFromString(classStr) alloc] init] forKey:propertyName];
        }
