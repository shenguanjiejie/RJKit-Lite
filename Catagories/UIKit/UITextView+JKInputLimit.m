...
- (void)swizzledDealloc {
    ...
    NSInteger length = [objc_getAssociatedObject(self, @selector(jk_maxLength)) integerValue];
    if (length) {
        @try {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
...
- (void)setJk_maxLength:(NSInteger)maxLength {
    ...
    if (maxLength) {
        [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(jk_textViewTextDidChange:) name:@"UITextViewTextDidChangeNotification" object:self];
    }
}
